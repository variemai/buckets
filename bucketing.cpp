...
constexpr int get_bucket_index(int64_t value, size_t index = NUM_BUCKETS - 1) {
    if (index == 0) {
        return ~(value - thresholds[0] >> 63);  // Handle the first bucket explicitly
    } else {
        return ~(value - thresholds[index - 1] >> 63) + get_bucket_index(value, index - 1);
    }
}
